/**
 * Tarefa Agendada: Notificação de Aniversariantes do Dia
 *
 * Execução: Diariamente às 08:00 (horário de Brasília)
 * Procedure tRPC: trpc.alertas.notificarAniversariantes
 *
 * Descrição:
 *   Este módulo executa a verificação de beneficiários que fazem aniversário
 *   no dia atual, formata uma mensagem com a lista de nomes e idades, e envia
 *   uma notificação ao owner através da função notifyOwner(). Retorna sucesso
 *   e quantidade de aniversariantes encontrados.
 *
 ...
 * Formato: logs/aniversariantes-YYYY-MM-DD.log
 * @returns {string}
 */
function caminhoLogDiario() {
  const hoje = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  return path.join(CONFIG.logDir, `aniversariantes-${hoje}.log`);
}

/**
 * Formata a data e hora atual no padrão brasileiro para uso nos logs.
 * @returns {string} Data e hora formatadas, ex.: "13/04/2026 08:00:00"
 */
function agora() {
 ...
 *
 * @param {string} procedure - Nome completo da procedure (ex.: "alertas.notificarAniversariantes").
...
/**
 * Executa a tarefa de notificação de aniversariantes do dia,
 * invocando a procedure tRPC `alertas.notificarAniversariantes`.
 *
 * Fluxo:
 *  1. Chama `trpc.alertas.notificarAniversariantes`.
 *  2. A procedure busca beneficiários cujo dia e mês de aniversário
 *     coincidem com a data atual.
 *  3. Formata mensagem com lista de nomes e idades
 *     (ex: "• João Silva (25 anos)").
 *  4. Envia notificação ao owner via notifyOwner() com título e conteúdo.
 *  5. Retorna sucesso e quantidade de aniversariantes encontrados.
 *     Se não houver aniversariantes, retorna "Nenhum aniversariante hoje".
 *
 * @returns {Promise<void>}
 */
async function executarTarefa() {
  log('INFO', '='.repeat(60));
  log('INFO', 'Iniciando tarefa: Notificação de Aniversariantes do Dia');
  log('INFO', `Data atual: ${new Date().toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' })}`);
  log('INFO', `Configuração: maxRetries=${CONFIG.maxRetries}, retryDelayMs=${CONFIG.retryDelayMs}ms`);
  log('INFO', '='.repeat(60));

  try {
    log('INFO', 'Chamando procedure: trpc.alertas.notificarAniversariantes');

    const resultado = await comRetry(
      () => chamarTrpc('alertas.notificarAniversariantes'),
      CONFIG.maxRetries,
      CONFIG.retryDelayMs
    );

    log('INFO', '-'.repeat(60));
    log('INFO', 'Tarefa concluída com sucesso. Resultado:');

    if (resultado.mensagem === 'Nenhum aniversariante hoje' || resultado.total === 0) {
      log('INFO', '  Nenhum aniversariante encontrado para hoje.');
    } else {
      log('INFO', `  Total de aniversariantes : ${resultado.total ?? 'N/A'}`);
      log('INFO', `  Notificação enviada      : ${resultado.sucesso ? 'Sim' : 'Não'}`);
    ...
    log('INFO', '-'.repeat(60));
    log('INFO', `Resposta completa: ${JSON.stringify(resultado)}`);
